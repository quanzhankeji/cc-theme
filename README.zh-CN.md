...
`0.2.0` 源码线正在准备首个正式 `CC Theme` 应用 Release。此前已公证的 `CC Theme Manager`
镜像早于本次改名，不能继续作为当前应用发布。正式应用名为 `CC Theme.app`；本地 DMG 文件名中的
空格经 GitHub 规范化后，线上下载名为 `CC.Theme_0.2.0_aarch64.dmg`。
