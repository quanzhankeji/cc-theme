...
**CC Theme 0.2.1 当前仅支持 Apple Silicon。**
...
当前正式应用版本为 [CC Theme 0.2.1](https://github.com/quanzhankeji/cc-theme/releases/tag/cc-theme-v0.2.1)。
正式应用名为 `CC Theme.app`；本地 DMG 文件名中的空格经 GitHub 规范化后，线上下载名为
`CC.Theme_0.2.1_aarch64.dmg`。

CC Theme 0.2.1 是用于验证签名在线 Adapter 更新流程的过渡基线。安装包内置的 CodeX
Adapter 仍为此前公开的 `26.715.31925-r1`；应用会通过签名 stable Catalog 发现、验证并安装
更新的官方 Adapter。内置 Adapter 只承担最后有效回退，不代表安装包中的每个 Engine 都是最新版。
...
| [`mac-doubao`](adapters/mac-doubao/README.md) | 已在文档所列豆包版本支持 CC Theme 应用与恢复 |
