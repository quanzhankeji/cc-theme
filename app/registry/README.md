...
The currently registered public IDs are exactly `mac-codex`, `mac-doubao`, and `mac-workbuddy`. The Registry does not publish aliases; a preserved but unregistered Adapter source is not part of the Manager product or runtime resources.
...
Registration and online publication are separate states. Stable Catalog sequence `000001` publishes the existing `mac-codex` and `mac-workbuddy` Release assets only; `mac-doubao` must not be advertised as downloadable until an exact signed Release asset is published and a later signed Catalog sequence names it.
