...
- 状态：Accepted / Amended for signed online delivery
...
第一阶段实现两者的 Schema、生成、离线验证、确定性和恶意负例；2026-07-20 修订加入签名发现、
缓存、受限下载与安全安装。首个带在线更新能力的 Manager 仍保留 bundle 内 bootstrap Engine，直到
干净安装、断网和撤回回归证明可以安全移除。
...
- Package 完整性由 manifest 中每个文件的 bytes + SHA-256 以及整包 SHA-256 证明；发布者身份由 App
  内置 Ed25519 根公钥验证的 `stable.json` 与 exact-commit Catalog 证明，不能把哈希误当成签名；
...
固定入口只读取仓库 `main` 上的小型 `stable.json`；它的 detached Ed25519 签名通过后，才能读取其
绑定的 exact-commit Catalog。Catalog 再绑定 exact Release tag、asset name、bytes、archive SHA-256
与顶层 `adapter.json` SHA-256。客户端不调用 `/releases/latest`，不内置 GitHub token，也不允许前端
提供 URL、路径、命令、参数或跳过验证开关。

Manager 保存最高 sequence，拒绝降级；元数据过期时不允许新下载，但已安装 Adapter 和已验证
bootstrap Engine 不受影响。网络失败使用仍在有效期内的签名 last-known-good 快照。回滚通过更高
sequence 指向旧安全资产，坏资产摘要进入 `revokedSha256`，不覆盖历史 Release。
...
- 不做无人值守自动安装；Manager 自动检查，但必须由用户点击下载；
- 首个在线版本不移除 bundle 内 Engine，也不改变现有真实 Mac apply 安全门禁；
- 小于 1.1 MiB 的当前 Adapter 首版不提供分块进度与取消；超时/失败会删除临时文件并保留现状；
