...
use crate::{
    ...
    themes::{seed_compiled_theme_into_library, valid_theme_id},
};
...
pub fn compile_and_seed_theme(
    ...
    write_private_json(
        &context_file,
        &build_compile_context_for(definition.id.as_str(), Some(client_state)),
    )?;
    ...
    seed_compiled_theme_into_library(definition, target_directory, &assets, theme_id)
        .map(Some)
        .map_err(Into::into)
}
...
fn build_compile_context_for(
        ...
        .filter(|capability| capability.adapter_id == selected_adapter_id)
    {
        ...
        let probe_passed = state.is_some_and(|state| {
            state.discovered
                && state.adapter_ready
                && matches!(state.signature.status, SignatureState::Verified)
        });
        let apply_allowed = probe_passed && capability.runtime_apply_available;
        ...
        adapters.insert(
            ...
            json!({
                ...
                "reasonCode": if apply_allowed { Value::Null } else { Value::String("runtime-apply-unavailable".to_string()) },
