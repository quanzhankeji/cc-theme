...
正整数 `adapterReleaseRevision` 区分，当前为 `1`。本轮制品身份固定为
`mac-workbuddy-5.2.6-r1-macos-arm64`，单一机器源见 `PROJECT_MANIFEST.json`。
...
当前源码包为 `mac-workbuddy-5.2.6-r1-macos-arm64.zip`，客户端包为
`cc-theme-mac-workbuddy-5.2.6-r1-macos-arm64.zip`。当前 r1 仍是未正式发布的开发制品，prepare
