...
assert.equal(controller.setPosition({ xPercent: 18, yPercent: 72 }), true);
assert.deepEqual(JSON.parse(JSON.stringify(controller.inspect().position)), { xPercent: 18, yPercent: 72 });
assert.equal(drawCalls.length, 2, "changing the canonical background position must redraw the static directional frame once");
assert.equal(controller.setPosition({ xPercent: 18, yPercent: 72 }), true);
assert.equal(drawCalls.length, 2, "reapplying the same position must remain idempotent");
assert.equal(controller.setPosition({ xPercent: -1, yPercent: 72 }), false);
windowObject.emit("pointermove", { clientX: 1000, clientY: 400 });
frames.shift()();
assert.equal(controller.inspect().frame, 4);
assert.equal(drawCalls.length, 3, "directional mode redraws only after the selected frame changes");
windowObject.emit("pointermove", { clientX: 1000, clientY: 400 });
frames.shift()();
...
assert.equal(liveRipple.setPosition({ xPercent: 27, yPercent: 63 }), true);
assert.deepEqual(JSON.parse(JSON.stringify(liveRipple.inspect().position)), { xPercent: 27, yPercent: 63 });
