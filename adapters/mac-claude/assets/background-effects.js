(() => {
  ...
  function createBackgroundEffect(options) {
    ...
    const position = options.position || { xPercent: 50, yPercent: 50 };
    ...
    return {
      ...
      inspect() {
        return { mode, desiredEnabled, active: ready && !fallback && motionAllowed(), ready, fallback, frame: currentFrame };
      },
      dispose() {
        if (disposed) return;
        disposed = true;
