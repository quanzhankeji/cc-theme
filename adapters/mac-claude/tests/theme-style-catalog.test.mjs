...
import { loadStyleCatalog, validateStyleOverrideValues } from "../scripts/theme-style-catalog.mjs";
...
assert(catalog.nativeRoles.some((role) => role.id === "settings-nav-item"));
assert.deepEqual(catalog.roleFamilies.find((family) => family.id === "overlay")?.variants,
  ["dialog", "menu", "listbox"]);
assert(renderer.includes('uiInterpreter.reconcileRoles()'));
...
try {
  ...
  await saveStyleOverrides(theme, catalog, values, { root: stateRoot });
  ...
  assert.equal(rebased.diagnosticCode, "local-overrides-rebased");
