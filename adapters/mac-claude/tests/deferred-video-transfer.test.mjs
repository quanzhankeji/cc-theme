...
import {
  ...
  transferDeferredMotionFallback,
...
try {
  ...
  const motionFallbackPath = path.join(temporary, "background-motion.gif");
  await fs.writeFile(motionFallbackPath, source);
  received.length = 0;
  initialized = false;
  committed = false;
  const motionFallbackStat = await fs.stat(motionFallbackPath);
  const motionResult = await transferDeferredMotionFallback(session, {
    motionFallbackPath,
    motionFallbackStat,
    theme: { id: "runtime-fixture" },
  });
  assert.deepEqual(motionResult, { transferred: true, bytes: source.length });
  assert.equal(initialized, true);
  assert.equal(committed, true);
  assert.deepEqual(Buffer.concat(received), source);
...
console.log("PASS: video, animated fallback, and directional atlas delivery use bounded CDP chunks and renderer-local Blobs.");
