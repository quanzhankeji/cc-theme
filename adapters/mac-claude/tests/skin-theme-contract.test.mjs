...
const example = JSON.parse(await fs.readFile(path.join(root, "tests", "fixtures", "minimal-theme.json"), "utf8"));
...
assert.equal(MAC_CLAUDE_ADAPTER_ID, "mac-claude");
assert.equal(MAC_CLAUDE_CONTRACT, SKIN_THEME_KIND);
...
assert.deepEqual(themeMediaNames(example), ["fixture.png"]);
assert.equal(normalized.appearance.newTaskLayout, undefined);
...
assert.equal(normalized.pet, undefined);
assert.throws(() => normalizeSkinTheme({ ...example, pet: {} }, "Pet theme"), /unsupported fields/);
...
const normalizedVideo = normalizeSkinTheme({
  ...
  backgroundMotionFallback: "ambient-motion.gif",
...
assert.equal(normalizedVideo.backgroundMotionFallback, "ambient-motion.gif");
assert.deepEqual(themeMediaNames({
  ...example,
  backgroundVideo: "ambient.mp4",
  backgroundMotionFallback: "ambient-motion.gif",
}), ["fixture.png", "ambient.mp4", "ambient-motion.gif"]);
assert.equal(schema.properties.backgroundMotionFallback.$ref, "#/$defs/motionImageFile");
assert.throws(
  () => normalizeSkinTheme({ ...example, backgroundMotionFallback: "remote.gif" }, "Fallback without video"),
...
assert.deepEqual(themeMediaNames(directionalSource), ["fixture.png", "background-directions.webp"]);
assert.throws(
