...
export const MAC_CLAUDE_ADAPTER_ID = "mac-claude";
export const MAC_CLAUDE_CONTRACT = SKIN_THEME_KIND;
export const MAC_CLAUDE_TARGET_PATH = "targets/macos/theme.json";

const TOP_LEVEL_KEYS = new Set([
  "kind", "id", "name", "image", "homeHeroImage", "backgroundVideo", "backgroundMotionFallback",
  "interactiveBackground", "colors",
...
const APPEARANCE_KEYS = new Set([
  "shellMode", "backgroundVideoPosterMode", "backgroundPosition", "homeHeroPosition",
  "backgroundVideoPosition", "backgroundVideoScrimOpacity", "backdropBlurPx", "backdropSaturation", "radiusScale",
]);
...
export function themeMediaNames(value, label = "Theme config") {
  ...
  return [
    ...
    optionalMediaName(theme.backgroundMotionFallback, "backgroundMotionFallback", label),
...
export function normalizeSkinTheme(value, label = "Theme config") {
  ...
  const backgroundMotionFallback = optionalMediaName(
    raw.backgroundMotionFallback,
    "backgroundMotionFallback",
    label,
  );
  if (backgroundMotionFallback && !backgroundVideo) {
    throw new Error(`${label} backgroundMotionFallback requires backgroundVideo`);
  }
  if (backgroundMotionFallback && !/^[^/\\\\]+\.gif$/i.test(backgroundMotionFallback)) {
    throw new Error(`${label} backgroundMotionFallback must be a local GIF filename`);
  }
  ...
  return {
    ...
    ...(backgroundMotionFallback ? { backgroundMotionFallback } : {}),
