...
try {
  ...
  await fs.writeFile(path.join(engine, "VERSION"), `${adapterVersion}\n`);
  ...
  assert.match(plist, new RegExp(`<key>CFBundleShortVersionString<\\/key>\\s*<string>${adapterVersion.replaceAll(".", "\\.")}<\\/string>`));
  ...
  assert.equal(plist.includes("CFBundleIconFile"), false,
    "the Adapter launcher must not derive its icon from theme artwork");
  await assert.rejects(fs.stat(path.join(output, "Contents", "Resources", "AppIcon.icns")), { code: "ENOENT" });
