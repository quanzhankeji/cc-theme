...
// These keys are backed up only. Skin follows the user's existing shell
// appearance and never forces light or dark.
...
function decodeStrictUtf8(bytes, label) {
  ...
  if (content.includes("\0")) throw new Error(`${label} contains NUL characters; nothing was changed.`);
  return content;
}

async function readBackupForRestore() {
  let expectedStat;
  try {
    expectedStat = await fs.lstat(backupPath);
  } catch (error) {
    if (error.code === "ENOENT") return null;
    throw new Error(`Could not inspect the theme backup: ${error.message}`);
  }
  if (expectedStat.isSymbolicLink() || !expectedStat.isFile()) {
    throw new Error("Theme backup must be a regular file, not a symbolic link; nothing was restored.");
  }

  let handle;
  try {
    handle = await fs.open(backupPath, constants.O_RDONLY | constants.O_NOFOLLOW);
  } catch (error) {
    throw new Error(`Could not open the theme backup safely: ${error.message}`);
  }
  try {
    ...
    if (
      ...
      throw new Error("Theme backup identity changed during this operation; nothing was restored.");
    }
    return await handle.readFile();
  } catch (error) {
    throw new Error(`Could not read the theme backup safely: ${error.message}`);
  } finally {
...
async function acquireConfigLock() {
  const lockPath = `${configPath}.skin.lock`;
  ...
  while (true) {
    ...
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error("Another Skin config operation is still running; try again shortly.");
      }
...
async function main() {
  ...
  if (mode === "install") {
    ...
    // All configured values are null today: installation records state but
    // intentionally leaves the user's chosen appearance untouched.
  ...
  const backupBytes = await readBackupForRestore();
  if (backupBytes === null) {
    console.log("No selective pre-install theme backup is present; nothing was restored.");
    return;
  }
  let backup;
  try {
