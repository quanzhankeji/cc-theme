...
import { normalizeThemePetReference, THEME_PET_MANIFEST_PATH, THEME_PET_SPRITESHEET_PATH } from "./skin-pet.mjs";
import { installThemePet, readThemePetBundle } from "./theme-pet-store.mjs";
...
// skin.theme adapter. Older compatible compilers emitted a minimal adapter and
// therefore dropped several supported editor fields. Reapply the
// allowlisted document values after package integrity has been verified so a
// user-authored data survives export -> import without accepting executable input.
function mergeDocumentEdits(theme, document, manifest, targetVersion) {
  ...
  if (document.application !== "codex" || document.platform !== "macos" || document.appVersion !== targetVersion) {
  ...
  if (!plainObject(adapter)
      ...
      || !["cards", "banner"].includes(adapter.newTaskLayout)
      || !["none", "image"].includes(adapter.backgroundVideoPosterMode)
      || typeof adapter.reduceParticles !== "boolean") {
...
async function validatePackage(extractedRoot) {
  ...
  if (manifest?.kind !== SKIN_PACKAGE_KIND || manifest?.target?.application !== "codex" || manifest?.target?.platform !== "macos") {
    throw new Error("Package does not target Codex for macOS");
  }
  const targetVersion = manifest.target.version ?? manifest.target.codexVersion;
  if (typeof targetVersion !== "string" || !targetVersion.trim()) throw new Error("Package does not declare a Codex target version");
  if (manifest.target.version && manifest.target.codexVersion && manifest.target.version !== manifest.target.codexVersion) {
    throw new Error("Package contains conflicting Codex target versions");
  }
  const adapter = manifest.adapters?.find((candidate) => candidate?.id === MAC_CODEX_ADAPTER_ID);
  if (!adapter || adapter.contract !== MAC_CODEX_CONTRACT || adapter.status !== "compiled" || adapter.targetPath !== MAC_CODEX_TARGET_PATH) {
    throw new Error("Package does not contain the required compiled Mac-CodeX adapter");
  }
  ...
  const targetConfig = byPath.get(MAC_CODEX_TARGET_PATH);
  ...
  theme = mergeDocumentEdits(theme, sourceDocument, manifest, targetVersion);
  assertSkinThemeIdentity(theme, "Compiled macOS theme");
  const petReference = normalizeThemePetReference(theme.pet, theme.id, "Compiled macOS theme");
  ...
  if (petReference) {
    const petManifestPath = `targets/macos/${THEME_PET_MANIFEST_PATH}`;
    const petSpritesheetPath = `targets/macos/${THEME_PET_SPRITESHEET_PATH}`;
    if (!byPath.has(petManifestPath) || !byPath.has(petSpritesheetPath)) {
      throw new Error("Compiled macOS theme pet is incomplete");
    }
    const petManifestEntry = manifest.files.find((file) => file.path === petManifestPath);
    const petSpritesheetEntry = manifest.files.find((file) => file.path === petSpritesheetPath);
    if (petManifestEntry?.mediaType !== "application/json" || petSpritesheetEntry?.mediaType !== "image/webp") {
      throw new Error("Compiled macOS theme pet has invalid media types");
    }
    await readThemePetBundle(targetRoot);
  } else if ([...byPath.keys()].some((entry) => entry.startsWith("targets/macos/pet/"))) {
    throw new Error("Package contains unreferenced macOS pet files");
  }
  await fs.writeFile(targetConfig.absolute, `${JSON.stringify(theme, null, 2)}\n`, { mode: 0o600 });
  return { manifest, theme, targetRoot, targetVersion };
}

async function installTheme(sourceRoot, activeThemeRoot, theme, petsRoot, petRecordsRoot) {
  const destination = path.resolve(activeThemeRoot);
  const parent = path.dirname(destination);
  const activeName = path.basename(destination);
  await fs.mkdir(parent, { recursive: true, mode: 0o700 });
  const staging = path.join(parent, `.${activeName}.importing.${process.pid}.${randomUUID()}`);
  const previous = path.join(parent, `.${activeName}.previous.${process.pid}.${randomUUID()}`);
  ...
  try {
    ...
    let pet;
    try {
      pet = await installThemePet({ themeDirectory: destination, petsRoot, recordsRoot: petRecordsRoot });
    } catch (error) {
      await fs.rm(destination, { recursive: true, force: true }).catch(() => {});
      if (hadPrevious) await fs.rename(previous, destination).catch(() => {});
      throw error;
    }
    // The new theme and its optional pet are committed at this point. A stale
...
async function main() {
  ...
  const petsRootArg = valueFor("pets-root");
  const petRecordsRootArg = valueFor("pet-records-root");
  if (!packageArg || !activeThemeRootArg) throw new Error("Usage: import-cc-theme.mjs --file <theme.cctheme> --active-theme-root <active-theme-dir> [--pets-root <~/.codex/pets> --pet-records-root <state-dir>]");
  ...
  try {
    ...
    const { theme, targetRoot, targetVersion } = await validatePackage(temporary);
    const { destination, pet } = await installTheme(
      targetRoot,
      activeThemeRoot,
      theme,
      petsRootArg ? path.resolve(petsRootArg) : null,
      petRecordsRootArg ? path.resolve(petRecordsRootArg) : null,
    );
    process.stdout.write(JSON.stringify({
      id: theme.id,
      name: theme.name,
      targetVersion,
      adapter: MAC_CODEX_ADAPTER_ID,
      destination,
      pet,
    }));
  } finally {
