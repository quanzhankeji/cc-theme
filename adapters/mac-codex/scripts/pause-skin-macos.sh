...
REMOVED="false"
# Drop any obsolete launchd job that would relaunch Codex with CDP after quit.
...
if [ "$REMOVED" = "true" ]; then
  printf 'CC Theme paused (skin removed; Codex left running). Port %s may still be in debug mode.\n' "$PORT"
elif codex_is_running; then
  printf 'CC Theme paused (injector stopped). Live remove skipped: CDP on port %s not verified.\n' "$PORT"
else
  printf 'CC Theme paused (Codex is not running).\n'
fi
