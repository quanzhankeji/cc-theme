...
$resultKind = if ($Mode -eq 'settings') { 'cc-theme.win-workbuddy-settings-structure-evidence' } else { 'cc-theme.win-workbuddy-layer-evidence' }
...
try {
    ...
    $script = Join-Path $PSScriptRoot 'layer-probe.mjs'
    $oldRunAsNode = $env:ELECTRON_RUN_AS_NODE
    try {
        $env:ELECTRON_RUN_AS_NODE = '1'
        & $owner.Path $script --port $Port --mode $Mode
    ...
    if ($code -notmatch '^[a-z][a-z0-9-]{1,79}$') { $code = 'layer-probe-failed' }
    [ordered]@{
        kind = $resultKind
        schemaVersion = 1
